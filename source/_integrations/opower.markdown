...
Depending on the utility and your setup, there may be multiple meters for a single utility account.  If this is the case, your sensors will
use a UUID to distinguish the data streams within that single account.  The UUID will not tie back to any identifier used by the utility.
...
Similar to sensors, if there are multiple meters for a single utility account, the statistics IDs and names will use a UUID to distinguish
the data streams within that single account.
